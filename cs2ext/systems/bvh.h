...
static bool extract_mesh( std::uintptr_t bvh_ptr, std::uintptr_t vert_ptr, std::uintptr_t tri_ptr, std::uint32_t node_count, const mat3_t& rot, const float scale[ 3 ], const float pos[ 3 ], std::uintptr_t mat_arr_ptr, std::int32_t mat_count, const std::vector<bvh::global_surface_entry>& global_table, const bvh::surface_info& default_surface, std::vector<bvh::triangle>& out )
{
	...
	g_memory->read_raw( bvh_ptr, bvh_buf.data( ), bvh_buf.size( ) );
	...
	g_memory->read_raw( tri_ptr + static_cast< std::uintptr_t >( min_tri ) * 12, indices.data( ), total_tris * 12 );
	...
	g_memory->read_raw( vert_ptr, vertices.data( ), static_cast< std::size_t >( vert_count ) * 12 );
	...
	if ( has_materials )
	{
		...
		g_memory->read_raw( mat_arr_ptr + static_cast< std::uintptr_t >( min_tri ), materials.data( ), total_tris );
	}
...
static bool extract_hull( std::uintptr_t hull_data, float uniform_scale, const bvh::surface_info& surface, std::vector<bvh::triangle>& out )
{
	...
	g_memory->read_raw( hull_data, hd, sizeof( hd ) );
	...
	g_memory->read_raw( vert_ptr, verts.data( ), static_cast< std::size_t >( vert_count ) * 12 );

	std::vector<hedge_t> hedges( hedge_count );
	g_memory->read_raw( hedge_ptr, hedges.data( ), static_cast< std::size_t >( hedge_count ) * 4 );

	std::vector<std::uint8_t> faces( face_count );
	g_memory->read_raw( face_ptr, faces.data( ), face_count );
...
{
	const auto vtable = g_memory->read<std::uintptr_t>( shape_body );

	if ( vtable == hull_vtable )
	{
		const auto hull_data = g_memory->read<std::uintptr_t>( shape_body + 0xb8 );
		if ( hull_data > 0x10000 && hull_data < 0x7fffffffffff )
		{
			const auto scale = g_memory->read<float>( shape_body + 0xb0 );

			bvh::surface_info hull_surface{};
			hull_surface.penetration = g_memory->read<float>( shape_body + 0x28 );
	...
	const auto mesh_data = g_memory->read<std::uintptr_t>( shape_body + 0xc0 );
	if ( !mesh_data )
	{
		return;
	}
	...
	default_surface.penetration = g_memory->read<float>( shape_body + 0x28 );

	const auto default_damage = g_memory->read<float>( shape_body + 0x2c );
	...
	std::uint8_t md[ 0xA0 ]{};
	g_memory->read_raw( mesh_data, md, sizeof( md ) );
	...
	float scale[ 3 ]{};
	g_memory->read_raw( shape_body + 0xB0, scale, 12 );
	...
	float world_pos[ 3 ]{};
	g_memory->read_raw( shape_body + 0x100, world_pos, 12 );

	quat_t quat{};
	g_memory->read_raw( shape_body + 0x130, &quat, sizeof( quat ) );
...
{
	const auto trace_against_entities_call = g_memory->find_pattern(g_memory->get_client_base(), "E8 ? ? ? ? C7 87 ? ? ? ? ? ? ? ? 48 8D 54 24 ? 48 8B CF" );
	...
	const auto get_surface_data_from_handle_fn = g_memory->find_pattern( g_memory->get_client_base(), "48 63 41 ? 48 8B 0D" );
	const auto surface_manager = g_memory->read<std::uintptr_t>( g_memory->resolve_rip( get_surface_data_from_handle_fn + 4 ) );
	...
	{
		const auto array_base = g_memory->read<std::uintptr_t>( surface_manager + 40 );
		if ( array_base )
		{
			...
			{
				const auto candidate = g_memory->read<std::int32_t>( surface_manager + off );
				if ( candidate > 0 )
				{
			...
			if ( surface_count <= 0 )
			{
				for ( int i = 0; i < 1024; ++i )
				{
					...
					g_memory->read_raw( array_base + static_cast< std::size_t >( i ) * 32, &sd, sizeof( sd ) );
			...
			if ( surface_count )
			{
				...
				g_memory->read_raw( array_base, global_table.data( ), static_cast< std::size_t >( surface_count ) * sizeof( global_surface_entry ) );
			}
		}
	}

	const auto inner_world = g_memory->read<std::uintptr_t>( vphys2_world + 0x30 );
	if ( !inner_world )
	{
		return;
	}

	const auto body_array = g_memory->read<std::uintptr_t>( inner_world + 0x110 );
	if ( !body_array )
	{
		return;
	}

	const auto body_count = g_memory->read<std::int32_t>( body_array + 0x268 );
	if ( !body_count )
	{
		return;
	}

	const auto hull_vtable = g_memory->find_vtable( g_memory->get_modules().vphysics2, "CRnHullShape" );
	const auto mesh_vtable = g_memory->find_vtable( g_memory->get_modules().vphysics2, "CRnMeshShape" );
	...
	for ( std::int32_t body_idx = 0; body_idx < body_count; ++body_idx )
	{
		...
		const auto bvh_root = g_memory->read<std::int32_t>( body );
		const auto bvh_nodes_ptr = g_memory->read<std::uintptr_t>( body + 0x18 );
		...
		const auto femboys = g_memory->read<std::uint32_t>( body + 0x40 );
		if ( femboys != 2 )
		{
			continue;
		}
		...
		if ( bvh_root >= 0 )
		{
			...
			const auto count_b = static_cast< std::uint32_t >( g_memory->read<std::int32_t>( body + 0x08 ) );
			const auto count_c = static_cast< std::uint32_t >( g_memory->read<std::int32_t>( body + 0x10 ) );
			...
			g_memory->read_raw( bvh_nodes_ptr, outer_buf.data( ), outer_buf.size( ) );
		...
		{
			const auto shape = g_memory->read<std::uintptr_t>( body + 0x28 );
			if ( shape )
			{
				process_shape( shape, hull_vtable, mesh_vtable, global_table, fresh );
			}
...
inline const std::vector<bvh::triangle>& bvh::triangles( ) const
{
	return this->m_triangles;
}

inline std::size_t bvh::count( ) const
{
	std::shared_lock lock( this->m_mutex );
	return this->m_triangles.size( );
}

inline bool bvh::valid( ) const
{
	std::shared_lock lock( this->m_mutex );
